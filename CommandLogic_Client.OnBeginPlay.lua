return function (self) 
	...
	Unknown.InitHack = function(self)
		if(not Unknown.Init)then
			...
			-- 부분부분 요소들을 관리하려하는 프레임단위.
			Unknown.Frame = 0
			Unknown.FrameSec = 0
			Unknown.ElapsedSec = 0
			Unknown._previousTime = 0 -- 공격용 체크 타이머
			Unknown._previousTime_buff_1 = -999 -- 버프용 체크 타이머
			Unknown._previousTime_buff_2 = -999 -- 버프용 체크 타이머
			Unknown._previousTime_buff_3 = -999 -- 버프용 체크 타이머
			Unknown._previousTime_buff_4 = -999 -- 버프용 체크 타이머
			
			-- 하드코딩 ( 필터 )
			
			Unknown.excludeItemIds = {4000058,4031013,4031165,4031164,4000083,4000084,4000085} -- 초기 아이템 ID 추가

			for i = 4130000, 4130023 do
				table.insert(Unknown.excludeItemIds, i) -- 범위 내의 아이템 ID를 테이블에 추가
			end

			for i = 4004000, 4004004 do
				table.insert(Unknown.excludeItemIds, i) -- 범위 내의 아이템 ID를 테이블에 추가
			end

			for i = 4010000, 4020008 do
				table.insert(Unknown.excludeItemIds, i) -- 범위 내의 아이템 ID를 테이블에 추가
			end
			...
			Unknown.Use_attack_speed_Value = 20
			...
			Unknown.Reset_All_Variable = function()
				...
				if(Unknown.Drop_timer ~= 0) then _TimerService:ClearTimer(Unknown.Drop_timer) end
				if(Unknown.Buff_timer ~= 0) then _TimerService:ClearTimer(Unknown.Buff_timer) end
				
				Unknown.Use_Filter_Arrow = false
				Unknown.Use_Filter_Etc = false
				Unknown.Use_Filter_Equip = false
				Unknown.Use_Not_diffrent_User_Item = false
				Unknown.Use_Pos = false
				Unknown.Use_Auto = false
				Unknown.Use_Auto_Distance = 1
				Unknown.Use_attack_speed_Value = 20
				Unknown.DetectInventory_2 = false
				Unknown.DetectInventory = false
				Unknown.Teleport_Mouse = false
				Unknown.Musi = false
				Unknown.just_drop = false
				Unknown.hide_name = false
				Unknown.Use_LogOut_and_Return = false
				Unknown.Use_LogOut = false
				Unknown.Use_Return = false
				Unknown.Use_attack_speed = false
			end
			...
			local hpPotions = {
				{id = 2000000, name = "빨간 포션", value = 50},
				{id = 2000001, name = "주황 포션", value = 150},
				{id = 2000002, name = "하얀 포션", value = 300},
			...
			local mpPotions = {
				...
				{id = 2010004, name = "레몬", description = "매우 신 과일이다.\nMP를 약 150 회복시킨다."},
			...
			Unknown.auto_potion_func = function(self)
				if(Unknown.Use_auto_potion) then
					local hp = _UserService.LocalPlayer.WsUser.Health
					local maxHp = _UserService.LocalPlayer.WsUser.MaxHealth
				...
				if (Unknown.Frame % 12 == 0) then
					if (Unknown.Use_Normal or Unknown.Use_Skill) and not Unknown.Some_People() then
						...
						for i,v in ipairs(output) do
							...
							if(Unknown.Leader ~= nil and _LocalPartyLogic.LocalParty ~= nil) then
								target = Unknown.Leader.Entity
							end
							mob.MobAIComponent.Target = target
							...
							-- pcall(function() mob.MobTemporaryStat:Clear(); end)
						end
					end
				end
			end
			...
			Unknown.Flying = function(mob)
				...
				local DirX = (target.PlayerControllerComponent.LookDirectionX < 0 and -1 or 1)
			...
			Unknown.Auto_Hunt_Func = function(self)
				...
				if #distanceTable > 0 then
					...
					while closest_mob.Entity.MobComponent.DeadState == -1 and (Unknown.Use_Normal or Unknown.Use_Skill) and not Unknown.Some_People() do
						...
						wait(Unknown.FrameSec)
						-- if(Unknown._Attack_Time and Unknown.Use_Skill) then break end
					end
				end
			...
			Unknown.Play_func = function(self)
					...
					if Unknown.Some_People()then
						if(Unknown.Use_Normal or Unknown.Use_Skill)then
							if(not Unknown.has_People) then
								...
								_TimerService:ClearTimer(Unknown.Drop_timer)
								if(Unknown.Use_buff) then
									_TimerService:ClearTimer(Unknown.Buff_timer)
								end
						...
						if(Unknown.Use_Normal or Unknown.Use_Skill)then
							if(Unknown.has_People)then
								...
								-- Unknown.Hit_timer = _TimerService:SetTimerRepeat(Unknown.Auto_Attack_func, 0.1,0.15)
								-- if(Unknown.just_drop and not Unknown.Use_Auto)then
									-- Unknown.Drop_Time = _UtilLogic.ElapsedSeconds
									-- Unknown.Drop_timer = _TimerService:SetTimerRepeat(Unknown.Drop_timer_Func, 3)
								-- end
								if(Unknown.Use_buff) then
									Unknown.Buff_timer = _TimerService:SetTimerRepeat(Unknown.Buff_timer_func, 1)
								end
								Unknown.has_People = false
				...
				elseif(Unknown.Use_Hide)then
					...
					if Unknown.Some_People() then
						if (Unknown.Use_Normal or Unknown.Use_Skill) and Unknown.Use_LogOut_and_Return then
							_TimerService:ClearTimer(Unknown.Hunt_timer)
							_TimerService:ClearTimer(Unknown.Hit_timer)
							_TimerService:ClearTimer(Unknown.Move_timer)
							_TimerService:ClearTimer(Unknown.Drop_timer)
							...
							_ChatMessageLogic:Add(_ChatMessageType.Red, "[감지 시스템] 사람이 감지되어 마을로 간후 로그아웃합니다.")
							_TimerService:SetTimerOnce(Unknown.Exigent_Func)
							...
							return
						elseif (Unknown.Use_Normal or Unknown.Use_Skill) and Unknown.Use_LogOut then
							_TimerService:ClearTimer(Unknown.Hunt_timer)
							_TimerService:ClearTimer(Unknown.Hit_timer)
							_TimerService:ClearTimer(Unknown.Move_timer)
							_TimerService:ClearTimer(Unknown.Drop_timer)
			...
			Unknown.Is_full_extitem = function(self)
				...
				if ExtSlotcount == cd.ItemMaxSlot[4] then
					if (Unknown.DetectInventory_2) then
						...
						Unknown.DetectInventory_2 = false;
						return
					elseif (Unknown.DetectInventory) then
						_UserService.LocalPlayer.RigidbodyComponent.IsQuaterViewMove = false
						_ItemConsumeLogic:TryConsumeFromFuncKey(2030000)
				...
				ExtSlotcount = 0
					for j = 1, cd.ItemMaxSlot[1], 1 do
					local item = cd:GetItem(1, j)
					if item ~= nil then
						ExtSlotcount = ExtSlotcount + 1
					end
				end
				if ExtSlotcount == cd.ItemMaxSlot[1] then
					if (Unknown.DetectInventory_2) then
						Unknown.Use_Filter_Etc = true;
						_ChatMessageLogic:Add(_ChatMessageType.System, "[자동감지] 인벤토리가 가득차서 아이템필터가 설정되었습니다.")
						Unknown.DetectInventory_2 = false;
			...
			Unknown.Auto_Attack_func = function(self)
				...
				if(Unknown.Use_attack_speed)then
					if (Unknown.ElapsedSec - Unknown._previousTime >= (0.03 * Unknown.Use_attack_speed_Value)) then
						_TimerService:SetTimerOnce(Unknown.Attack_Func)
						Unknown._previousTime = Unknown.ElapsedSec
					end
				else
					local time = _UtilLogic.ElapsedSeconds
					Unknown._Attack_Time = false
					local playRate = (_UserService.LocalPlayer.CharacterActionComponent:GetWeaponAttackSpeed(false) + 10) / 16
					if (time - Unknown.Attack_Time) >= playRate and not Unknown._Attack_Time then
						Unknown.Attack_Time = time
						_TimerService:SetTimerOnce(Unknown.Attack_Func)
						Unknown._Attack_Time = true
					end	
				end
			end
			...
			Unknown.Auto_Move_func = function(self)
				...
				local Target_Position = (_UserService.LocalPlayer.TransformComponent.WorldPosition + Vector3(0.2,0.5,0)):ToVector2()
				if(not Unknown._isLeftMove) then
					Target_Position = (_UserService.LocalPlayer.TransformComponent.WorldPosition - Vector3(0.2,-0.5,0)):ToVector2()
				end
				...
				_UserService.LocalPlayer.MovementComponent:SetWorldPosition(Target_Position)
			end
			...
			Unknown.Buff_timer_func = function(self)
				...
				if (Unknown.Use_buff) then
					...
					if (job == 2) then
						if (Unknown.ElapsedSec - Unknown._previousTime_buff_1 >= 200 and not Unknown.using_buff)then
							Unknown._previousTime_buff_1 = Unknown.ElapsedSec
						...
						if (Unknown.ElapsedSec - Unknown._previousTime_buff_2 >= 600 and not Unknown.using_buff)then
							Unknown._previousTime_buff_2 = Unknown.ElapsedSec
							local SkillID = 2001002
							Unknown.using_buff = true
							wait(1)
							_PlayerActiveSkillLogic:DoActiveSkill(SkillID, false)
							wait(1)
					...
					elseif (cd.Job == 110) then
						if(Unknown.ElapsedSec - Unknown._previousTime_buff_3 >= 160 and not Unknown.using_buff)then
							Unknown._previousTime_buff_3 = Unknown.ElapsedSec
					...
					elseif (job == 3) then
						...
						if(Unknown.buff_ < 5 and not Unknown.using_buff)then
							local SkillID = tonumber((cd.Job * 10000) + 1004)
							local slv = cd:GetSkillLevel(SkillID)
							Unknown.buff_ = slv * 30
							Unknown.using_buff = true
							wait(2)
							_PlayerActiveSkillLogic:DoActiveSkill(SkillID, false)
							wait(1)
...
self.ClientCommands = {
	...
	["?"] = function(self)
		if(Unknown.Init)then
			...
			[기타]
				...
				/공속								/위치 {위치시스템} 								/하이드
				/메소 [횟수] {50000x횟수 드랍}	/무시 {사람 있어도 작동} 		/줍기				/귀아 {귀환 + 로그아웃}
	...
	["메소"] = function(self,user,argc,args)
		if(Unknown.Init)then
			...
			if(tonumber(args[2]) > 200) then 
				_ChatMessageLogic:Add(_ChatMessageType.Creator, "!! 200미만의 횟수만 가능합니다. !!")
				return 
			end
	...
	["접속"] = function(self)
		if(Unknown.Init)then
			...
			if(Unknown.Use_LogOut_and_Return) then _ChatMessageLogic:Add(_ChatMessageType.Blue, "[감지 시스템] 이미 로그아웃이 설정 되어 있습니다.") return end
	...
	["아귀"] = function(self)
		if(Unknown.Init)then
			...
			if(Unknown.Use_Return) then _ChatMessageLogic:Add(_ChatMessageType.Blue, "[감지 시스템] 이미 로그아웃이 설정 되어 있습니다.") return end
			Unknown.Use_LogOut_and_Return = not Unknown.Use_LogOut_and_Return
			-- Unknown.ignore_people = not Unknown.ignore_people
			local strings = Unknown.Use_LogOut_and_Return and "설정" or "설정해제"
			local color = Unknown.Use_LogOut_and_Return and _ChatMessageType.Yellow or _ChatMessageType.Red
			_ChatMessageLogic:Add(color, "[감지 시스템] 귀환 + 로그아웃 설정 " .. strings .. " 되었습니다.")
		end
	end,
	["귀환"] = function(self)
		if(Unknown.Init)then
	...
	["return"] = function(self)
		if(Unknown.Init)then
			...
			if(Unknown.Use_LogOut_and_Return) then _ChatMessageLogic:Add(_ChatMessageType.Blue, "[Detection System] It is already set.") return end
			Unknown.Use_Return = not Unknown.Use_Return
			-- Unknown.ignore_people = not Unknown.ignore_people
	...
	["인벤토리"] = function(self,user,argc,args)
		if(Unknown.Init)then
			if(argc < 2)then _ChatMessageLogic:Add(_ChatMessageType.Yellow, "[인벤토리] 명령어 : /인벤토리 {귀환, 필터}") end
			if(args[2] == "귀환")then
				...
				_ChatMessageLogic:Add(color, "[인벤토리] 기타 + 장비창 인식 시스템 " .. strings .. " 되었습니다. [ 귀환 ]")
				return
			elseif(args[2] == "필터")then
				...
				_ChatMessageLogic:Add(color, "[인벤토리] 기타 + 장비창 인식 시스템 " .. strings .. " 되었습니다. [ 필터 ]")
				return
			end
		end
	end,

return function (self,event) 
	...
	if(Unknown.Init)then
		if(KeyboardKey.F1 == key) then
			...
			_ChatMessageLogic:Add(Color, "[단축키] 자동 사냥(스킬) " .. strings)
		elseif(KeyboardKey.F2 == key) then
			...
			_ChatMessageLogic:Add(Color, "[단축키] 자동 사냥(평타) " .. strings)
		elseif(KeyboardKey.F3 == key) then -- 
			...
			local color = Unknown.Use_setting_full and _ChatMessageType.Yellow or _ChatMessageType.Red
			_ChatMessageLogic:Add(color, "[단축키]세팅 - " .. strings)
		elseif(KeyboardKey.F4 == key) then -- 
			...
			local color = Unknown.Use_buff and _ChatMessageType.Yellow or _ChatMessageType.Red
			_ChatMessageLogic:Add(color, "[단축키]자동버프 - " .. strings)
		elseif(KeyboardKey.F7 == key) then
			Unknown.Use_Auto = not Unknown.Use_Auto
			local strings =  Unknown.Use_Auto and "자동사냥 모드" or "몹몰이 모드"
			local color = Unknown.Use_buff and _ChatMessageType.Yellow or _ChatMessageType.Red
			_ChatMessageLogic:Add(color, "[단축키]" .. strings .. " 선택되었습니다.")
			...
			if hit > 0 then
				...
				while not moved and Unknown.Use_Portal_Number <= hit do
					...
					if tostring(portalId) ~= "m999999999" and (portalType == _PortalType.Visible or portalType == _PortalType.Invisible) then
						...
						_ChatMessageLogic:Add(_ChatMessageType.Creator, "[포탈이동] " .. Unknown.Use_Portal_Number .. "번 포탈로 이동했습니다.")
						moved = true
					end
